...
function HomeArchiveSections({ sections }: { sections: HomeTimeSection[] }) {
  if (!sections.length) return null

  const [spotlight, collection, indexSection] = sections
  const leadPost = spotlight?.posts[0]
  const sidePosts = spotlight?.posts.slice(1, 5) || []
  const collectionPosts = collection?.posts || []
  const indexPosts = indexSection?.posts || []

  return (
    <section className="relative -mt-px overflow-hidden bg-[linear-gradient(180deg,#F9EBE0_0%,#fffaf2_22%,#fffaf2_100%)] text-[#2f1d16]">
      <div className="mx-auto max-w-7xl px-4 pt-10 pb-16 sm:px-6 lg:px-8">
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#b76e45]">More from the site</p>
            <h2 className="mt-3 text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">Explore a wider mix of stories, guides, profiles, and useful posts.</h2>
          </div>
          <p className="max-w-2xl text-sm leading-8 text-[#72594a]">
            The homepage now pulls multiple backend streams and presents them as different content experiences, not repeated cards. It should feel like a full site with depth, variety, and plenty to browse.
          </p>
        </div>

        {spotlight && leadPost ? (
          <div className="mt-12 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            <Link href={getTaskHref(spotlight.task, leadPost.slug)} className="group overflow-hidden rounded-[2.6rem] border border-[#dbc6b6] bg-[#2f1d16] text-[#fff4e4] shadow-[0_28px_80px_rgba(47,29,22,0.20)]">
              <div className="relative min-h-[520px] overflow-hidden lg:min-h-[640px]">
                <ContentImage src={getPostImage(leadPost)} alt={leadPost.title} fill className="object-cover opacity-80 transition duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(47,29,22,0.08)_0%,rgba(47,29,22,0.58)_58%,rgba(47,29,22,0.86)_100%)]" />
                <div className="absolute bottom-0 left-0 right-0 p-7 sm:p-9">
                  <p className="text-[11px] font-bold uppercase tracking-[0.26em] text-[#e2c884]">{spotlight.eyebrow}</p>
                  <h3 className="mt-3 max-w-2xl text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">{leadPost.title}</h3>
                  <p className="mt-4 max-w-xl text-sm leading-7 text-[#fff4e4]/78">{leadPost.summary || spotlight.description}</p>
                </div>
              </div>
            </Link>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
              {sidePosts.map((post, index) => (
                <Link key={post.id} href={getTaskHref(spotlight.task, post.slug)} className="group grid gap-4 rounded-[1.8rem] border border-[#dbc6b6] bg-white p-4 shadow-[0_18px_50px_rgba(47,29,22,0.07)] transition hover:-translate-y-0.5 hover:shadow-[0_22px_60px_rgba(47,29,22,0.11)] sm:grid-cols-[120px_1fr]">
                  <div className="relative h-28 overflow-hidden rounded-[1.2rem] bg-[#f3e8db]">
                  ...
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[#b76e45]">Pick {index + 1}</p>
                    <h4 className="mt-2 line-clamp-2 text-lg font-semibold tracking-[-0.02em]">{post.title}</h4>
                    <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#72594a]">{post.summary || 'Open this post for more details.'}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        {collection && collectionPosts.length ? (
          <div className="mt-16 rounded-[2.6rem] border border-[#dbc6b6] bg-[#f3e8db] p-5 sm:p-7">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.26em] text-[#8b6d5a]">{collection.eyebrow}</p>
                <h3 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">{collection.title}</h3>
              </div>
              <Link href={collection.href} className="inline-flex items-center gap-2 rounded-full border border-[#2f1d16]/15 bg-white px-5 py-3 text-sm font-semibold text-[#2f1d16] hover:bg-[#fffaf2]">
                Browse all
                <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
            <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {collectionPosts.map((post, index) => (
                <Link key={post.id} href={getTaskHref(collection.task, post.slug)} className={index % 3 === 0 ? 'rounded-[2rem] bg-[#2f1d16] p-5 text-[#fff4e4] shadow-[0_18px_50px_rgba(47,29,22,0.16)]' : 'rounded-[2rem] border border-[#dbc6b6] bg-white p-5 text-[#2f1d16] shadow-sm'}>
                  <span className={index % 3 === 0 ? 'text-[11px] font-bold uppercase tracking-[0.22em] text-[#e2c884]' : 'text-[11px] font-bold uppercase tracking-[0.22em] text-[#b76e45]'}>Reading lane</span>
                  <h4 className="mt-4 line-clamp-3 text-xl font-semibold tracking-[-0.03em]">{post.title}</h4>
                  <p className={index % 3 === 0 ? 'mt-4 line-clamp-3 text-sm leading-7 text-[#fff4e4]/72' : 'mt-4 line-clamp-3 text-sm leading-7 text-[#72594a]'}>{post.summary || collection.description}</p>
                </Link>
              ))}
            </div>
          </div>
        ...
        {indexSection && indexPosts.length ? (
          <div className="mt-16">
            <div className="flex flex-col gap-4 rounded-[2rem] border border-[#dbc6b6] bg-white p-7 shadow-[0_18px_50px_rgba(47,29,22,0.07)] sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.26em] text-[#b76e45]">{indexSection.eyebrow}</p>
                <h3 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">{indexSection.title}</h3>
              </div>
              <p className="max-w-2xl text-sm leading-8 text-[#72594a]">{indexSection.description}</p>
            </div>
            <div className="mt-6 columns-1 gap-4 sm:columns-2 lg:columns-3 xl:columns-4">
              {indexPosts.map((post, index) => (
                <Link key={post.id} href={getTaskHref(indexSection.task, post.slug)} className="mb-4 block break-inside-avoid rounded-[1.6rem] border border-[#dbc6b6] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-[0_18px_48px_rgba(47,29,22,0.10)]">
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#8b6d5a]">No. {String(index + 1).padStart(2, '0')}</p>
                  <h4 className="mt-3 text-lg font-semibold leading-snug tracking-[-0.02em]">{post.title}</h4>
                  <p className="mt-3 line-clamp-4 text-sm leading-7 text-[#72594a]">{post.summary || 'A compact index item from the wider site archive.'}</p>
                </Link>
              ))}
            </div>
          </div>
...
export default async function HomePage() {
  ...
  const taskFeed: TaskFeedItem[] = await fetchHomeTaskFeed(8, { timeoutMs: 2500 })

  const primaryTask = enabledTasks.find((task) => task.key === recipe.primaryTask) || enabledTasks[0]
  const homeTimeSections = primaryTask ? await fetchHomeTimeSections(primaryTask.key, { limit: 8, timeoutMs: 4000 }) : []
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {productKind === 'editorial' ? (
        <ReaderHome primaryTask={primaryTask} articlePosts={articlePosts} profilePosts={profilePosts} supportTasks={supportTasks} />
      ) : null}
      {productKind === 'visual' ? (
      ...
      <HomeArchiveSections sections={homeTimeSections} />
      <Footer />
    </div>
  )
}
